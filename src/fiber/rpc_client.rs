//! Lightweight JSON-RPC 2.0 client — vendored from fiber-cli.
//!
//! Source: `~/Cryptape/fiber/lightning-network/fiber/crates/fiber-cli/src/rpc_client.rs`
//!
//! This module is used to communicate with the Fiber Network Node (FNN)
//! JSON-RPC API. The `RpcClient` struct wraps `reqwest::Client` with
//! URL auto-detection, auth token support, and typed RPC helpers.
//!
//! The `FiberRpcExt` extension trait adds `AppError`-returning wrappers
//! so callers don't need to convert `anyhow::Error` at every call site.
...
// ---------------------------------------------------------------------------
// AppError extension — wraps anyhow errors for Opticrum server use
// ---------------------------------------------------------------------------

use crate::error::AppError;

/// Extension trait that provides `AppError`-returning wrappers for `RpcClient`.
///
/// These methods map `anyhow::Error` → `AppError::ChainError` so callers
/// in the chain provider can use `?` directly without manual conversion.
#[async_trait::async_trait]
pub trait FiberRpcExt {
    /// Typed RPC call with a single serializable params object.
    async fn call_fiber<P, R>(&self, method: &str, params: &P) -> Result<R, AppError>
    where
        P: Serialize + Sync,
        R: DeserializeOwned;

    /// Typed RPC call with no params.
    async fn call_fiber_no_params<R>(&self, method: &str) -> Result<R, AppError>
    where
        R: DeserializeOwned;
}
...
impl FiberRpcExt for RpcClient {
        ...
        R: DeserializeOwned,
    {
        self.call_typed(method, params)
            .await
            .map_err(|e| AppError::ChainError(format!("Fiber RPC {}: {}", method, e)))
    }
    ...
    where
        R: DeserializeOwned,
    {
        self.call_typed_no_params(method)
            .await
            .map_err(|e| AppError::ChainError(format!("Fiber RPC {}: {}", method, e)))
    }
}
