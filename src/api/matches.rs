...
pub async fn scan_chain(state: web::Data<AppState>) -> Result<HttpResponse, AppError> {
    ...
    let items: Vec<MatchScanItem> = matches
        ...
        .map(|m| MatchScanItem {
            ...
            order_tx_hash: hex::encode(m.match_args.order_args.fiber_pubkey),
            order_output_index: 0, // order outpoint is embedded in match_args
            fiber_pubkey: hex::encode(m.match_args.order_args.fiber_pubkey),
            ...
            rent_per_block: m.match_data.rent_per_block,
            escrow_blocks: m.match_data.escrow_blocks,
