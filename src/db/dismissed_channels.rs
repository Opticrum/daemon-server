...
//!
//! Closed channels stay in the Fiber node's `list_channels` response until the
//! funds fully settle on chain, so "deleting" a channel in the console is a
//! display dismissal, not a chain mutation. This module stores the tombstone.
...
pub fn is_dismissed(conn: &mut SqliteConnection, channel_id: &str) -> Result<bool, AppError> {
    ...
    select(exists(
        dismissed_fiber_channels::table.filter(dismissed_fiber_channels::channel_id.eq(channel_id)),
    ))
    .get_result(conn)
    .map_err(AppError::from)
}
