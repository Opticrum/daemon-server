...
pub struct RuntimeConfig {
    ...
    /// CKB address used for automated signing (auto-match). Empty = first unlocked key.
...
impl RuntimeConfig {
    ...
    pub fn apply_partial(&mut self, partial: &RuntimeConfigPartial) {
        ...
        if let Some(v) = partial.confirm_count {
            self.confirm_count = v;
        }
        ...
        if let Some(v) = &partial.automation_signer_address {
            self.automation_signer_address = v.clone();
        }
        if let Some(v) = partial.chain_cache_enabled {
            self.chain_cache_enabled = v;
        }
